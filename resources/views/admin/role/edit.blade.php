                ...
                <div class="white_box mb_30">
                    ...
                    <form action="{{route('admin.role.update',['id' => $role->id])}}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('display_name') is-invalid @enderror" name="display_name" rows="4">{{$role->display_name}}</textarea>
                            @error('display_name')
                            <div class="message-error">{{ $message }}</div>
                            @enderror
                        </div>
                                        ...
                                        @foreach ($parent->permissionChildrent as $child)
                                            <div class="card-body text-primary col-md-3">
                                                <h5 class="card-title">
                                                    <input
                                                        id="{{ $child->id }}"
                                                        {{ $permissionsChecked->contains('id',$child->id) ? 'checked' : '' }}
