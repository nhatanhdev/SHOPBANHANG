                ...
                <div class="white_box mb_30">
                    ...
                    <form action="{{route('admin.product.update',['id' => $product->id])}}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="default_sel mb_30 w-100" name="category_id" style="display: none;">>
                                {!! $htmlOption !!}
                            </select>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control select2attr" name="attrs[]" style="width: 100%" multiple="multiple">
                                ...
                                <option value="{{ $item->id }}" {{ in_array($item->id, $array_parent) ? 'selected' : '' }}>
                                    {{ $item->name }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="feature_image">
                            <img src="{{($product->feature_image)}}" alt="" width="200px" height="200px">

                        </div>
                        @endif
                        <label class="form-label mb-3 mt-2">Ảnh đặc trưng</label>
                        <div class="input-group mb-5">
                            <input type="file" class="form-control" name="feature_image">
                        </div>
                        @if($product->product_images)
                        <div class="detail_images mb-1">
                                @foreach ($product->product_images as $image )
                                <img src="{{($image->image_path)}}" alt="" width="100px" height="100px">
                                @endforeach
                        </div>
                        @endif

                        <label class="form-label mb-2 mt-2">Ảnh chi tiết</label>
                        <div class="input-group">
                            <input type="file" class="form-control" name="detail_image[]" multiple>
                        </div>
                        <label class="form-label mb-2 mt-4">Mô tả</label>
                        <div class="input-group mb-3">
                            <textarea class="form-control my-editor" name="description" rows="15">{{$product->description}}
                            </textarea>
...
<script>
    ...
    $('.select2attr').change(function() {
        ...
        Variant_options(id_parent);

    });
    $('.select2attr').each(function(){
        var id_parent = $(this).val();
        Variant_options(id_parent);

    });
    function Variant_options(id_parent){
...
<script>
    $(document).ready(function() {
        ...
        function From_variants(id_parent){
            var product_id = $('#product_id').val();
            ...
            $.ajax({
                ...
                success: function(response) {
                    var chil_ids = Object.values(response.chil_ids);
                    var selectsHtml = '';
                    ...
                    $.each(response.items, function(index, item) {
                        ...
                        // Add options
                        $.each(item.attributes, function(index, attribute) {
                            formHtml += `<option value="${attribute.id}" ${chil_ids.includes(attribute.id.toString()) ? 'selected' : ''}>${attribute.name}</option>`;
                        });
        ...
        function updateTable(Arr_Variants = "") {
            var prices = [];
            var price_olds =[];
            var quantities = [];
            var images = [];
            if(Arr_Variants != ""){
                ...
                Arr_Variants.forEach(function(variant) {
                    prices.push(variant.price);
                    price_olds.push(variant.price_old);
                    quantities.push(variant.quantity);
                    images.push(variant.detail_image);
                });
            }
            var variants = [];
            ...
            $.each(combinations, function(index, combination) {
                ...
                variant['Trạng Thái (Kho)'] = '<input type="text" class="form-control" name="variants_status[]" value="' + (quantities[index] !== undefined && quantities[index] !== null ? quantities[index] : '1') + '">';
                variant['Giá Cũ'] = '<input type="text" class="form-control" name="variants_price_old[]" value="'+ (price_olds[index] !== undefined && price_olds[index] !== null ? price_olds[index] : '') + '">';
                variant['Giá Mới'] = '<input type="text" class="form-control" name="variants_price[]" value="'+ (prices[index] !== undefined && prices[index] !== null ? prices[index] : '') +'">';
                variant['Ảnh'] = '<input type="file" class="form-control" name="variants_image_gallery[]" multiple><input type="hidden" name="images[]" value="' + (images[index] !== undefined && images[index] !== null ? images[index] : '') + '">';
